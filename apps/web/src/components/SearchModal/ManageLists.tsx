...
import { ETH_URLS, UNSUPPORTED_LIST_URLS } from 'config/constants/lists'
  ...
  const sortedLists = useMemo(() => {
    ...
    return listUrls
      .filter((listUrl) => {
        ...
        if (isValid) {
          return (
            (chainId === ChainId.ETHEREUM && (ETH_URLS as string[]).includes(listUrl)) ||
            (chainId === ChainId.ARBITRUM && (ETH_URLS as string[]).includes(listUrl))
          )
        }
