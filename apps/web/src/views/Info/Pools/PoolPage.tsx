...
const PoolPage: React.FC<React.PropsWithChildren<{ address: `0x${string}` }>> = ({ address: routeAddress }) => {
  ...
  const address = routeAddress.toLowerCase() as `0x${string}`
  ...
  const { data: farmConfig } = useSWRImmutable(isStableSwap && chainId ? [`info/gerFarmConfig/${chainId}`] : null, () =>
