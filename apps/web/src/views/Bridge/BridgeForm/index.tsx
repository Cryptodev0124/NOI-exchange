...
const getPID = (symbol?: string, chainId?: number) => {
  if (symbol === "MATIC") {
    return chainId === ChainId.ARBITRUM ? {pid: 0n, isNative: true} : {pid: 0n, isNative: false}
  }
  if (symbol === "KNB") {
    return chainId === ChainId.ARBITRUM ? {pid: 1n, isNative: true} : {pid: 1n, isNative: false}
  }
  if (symbol === "XKR") {
    return {pid: 2n, isNative: false}
  }
  if (symbol === "USDT") {
    return {pid: 3n, isNative: false}
  }
...
export const BridgeForm: React.FC<Record<string, never>> = () => {
  ...
  } = useDerivedBridgeInfoWithStableSwap(independentField, typedValue, inputCurrency, outputCurrency, poolInfo?.[3].toString(), poolInfo?.[4])
    ...
    <>
      ...
      <Wrapper id="swap-page" style={{ minHeight: '412px' }}>
        <AutoColumn gap="sm">
          ...
          <CurrencyInputPanelForBridge
            label={(independentField === Field.OUTPUT) ? t('From') : ""}
            value={formattedAmounts[Field.INPUT] ? (Number(formattedAmounts[Field.INPUT]) * (10000-Number(fee)) / 10000).toString() : formattedAmounts[Field.INPUT]}
            showMaxButton={false}
            maxAmount={maxAmountInput}
