...
export function VerifyTokenForm({
  ...
  const handleNext = async () => {
    setTokenData({
      ...
      tokenDecimals: searchToken?.decimals ?? 0,
  ...
  useEffect(() => {
    setCurrency(shimmer2Tokens.smr)
  }, [chainId])
      ...
      <FormContainer>
        ...
        <Box>
          ...
          <Box>
            ...
            {listingOption && <Text color="text" fontSize="14px">{t("For auto listing, after you finalize the pool your token will be auto listed on CyberGlow DEX.", {symbol: currency?.symbol ?? ""})}</Text>}
          </Box>
        </Box>
        {currency?.wrapped.address !== shimmer2Tokens.smr.address && <Message variant="warning" icon={false} p="8px 12px">
          <MessageText color="text">
            <span>{t('Do not use this currency for auto liquidity tokens, or tokens that depend on SMR pair. It will lead to error when finalizing the pool or transfering the tokens (for example Liquidity Generator Token, BabyToken, Buyback Baby Token).')}</span>
          </MessageText>
        </Message>}
