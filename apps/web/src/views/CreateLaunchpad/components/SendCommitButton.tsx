...
interface SendCommitButtonPropsType {
  ...
  approveCallbackForFee: () => Promise<any>
  ...
  const handleCommit = async () => {
    const receipt = await fetchWithCatchTxError(() => onCreateLaunchpad(
      [
        ...
        new BigNumber(deFiData?.vestingData?.vestingFirst ?? 0).times(100).toFixed(),
        new BigNumber(deFiData?.vestingData?.vestingPeriod ?? 0).times(24*3600).toFixed(),
        new BigNumber(deFiData?.vestingData?.vestingEach ?? 0).times(100).toFixed()
      ], 
      [
        tokenData.tokenAddress,
        tokenData.currency.isNative ? ZERO_ADDRESS : tokenData.currency.wrapped.address,
        deFiData.refundType ? DEAD_ADDRESS : account
      ],
      [
        socials.description,
        socials.logoUrl,
