...
export function ReviewForm({
  ...
  } = useAccountInfo((Number(serviceFee) / 10**GTOKEN[chainId].decimals).toString(), GTOKEN[chainId])

  const {approvalState, approveCallback} = useApproveCallback(parsedAmount, getLaunchpadFactoryAddress(chainId))
  const {approvalState: approvalForFee, approveCallback: approveCallbackForFee} = useApproveCallback(parsedAmountForFee, getLaunchpadFactoryAddress(chainId))
