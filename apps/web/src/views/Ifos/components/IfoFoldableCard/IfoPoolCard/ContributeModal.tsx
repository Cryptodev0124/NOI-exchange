...
const ContributeModal: React.FC<React.PropsWithChildren<Props>> = ({
  ...
  const { isApproving, isApproved, isConfirmed, isConfirming, handleApprove, handleConfirm } =
    useApproveConfirmTransaction({
      ...
      onApprove: () => {
        return callWithGasPrice(raisingTokenContractApprover, 'approve', [contract?.address ?? "0x", MaxUint256], {
          gasPrice,
        })
      },
      ...
      onConfirm: () => {
        return callWithGasPrice(
          contract,
          'depositPool',
          [BigInt(valueWithTokenDecimals.integerValue().toString()), poolId === PoolIds.poolBasic ? 0 : 1],
          {
            value: BigInt(valueWithTokenDecimals.times(getFullDecimalMultiplier(12)).toString()),
            gasPrice,
          },
        )
      },
  ...
  const isWarning =
    valueWithTokenDecimals.isGreaterThan(userCurrencyBalance) || valueWithTokenDecimals.isGreaterThan(maximumTokenEntry ?? BIG_ZERO)
      ...
      <ModalBody maxWidth="360px">
        <Box p="2px">
          <Flex justifyContent="space-between" mb="16px">
            ...
            <Text>{label}</Text>
            <Text>{`${formatNumber(getBalanceAmount(maximumTokenEntry ?? BIG_ZERO, 18).toNumber(), 3, 3)} ${
              ifo.currency.symbol
            }`}</Text>
          </Flex>
              ...
              <Image
                src={
                  ifo.currency.symbol === 'CAKE'
                    ? '/images/cake.svg'
                    : `/images/1/tokens/0xC02aaA39b223FE8D0A0e5C4F27eAD9083C756Cc2.png`
                }
                width={24}
                height={24}
              />
