...
const IfoCardDetails: React.FC<React.PropsWithChildren<IfoCardDetailsProps>> = ({
  ...
  let version3MaxTokens = walletIfoData.ifoCredit?.creditLeft
    ? // if creditLeft > limit show limit else show creditLeft
      walletIfoData.ifoCredit.creditLeft.gt(
        poolCharacteristic?.limitPerUserInLP.minus(walletCharacteristic?.amountTokenCommittedInLP || BIG_ZERO) || BIG_ZERO,
      )
  ...
  const maxLpTokens =
    (ifo.version === 3 || (ifo.version >= 3.1 && poolId === PoolIds.poolUnlimited)) && ifo.isActive
      ? version3MaxTokens
        ...
        : 0
      : getBalanceNumber(poolCharacteristic?.limitPerUserInLP ?? BIG_ZERO, 18)
  ...
  const totalCommittedPercent = poolCharacteristic?.totalAmountPool
    ...
    .times(100)
    .toFixed(2)
  const totalLPCommitted = getBalanceNumber(poolCharacteristic?.totalAmountPool ?? BIG_ZERO, 18)
  ...
  const pricePerTokenWithFeeNumber = pricePerTokenWithFeeToOriginalRatio?.times(ifo.tokenOfferingPrice ?? BIG_ONE).toNumber()
    ...
    if (status === 'live') {
      return (
        <>
          ...
          <FooterEntry label={t('Funds to raise:')} value={ifo[poolId]?.raiseAmount} />
          {/* {raisingTokenToBurn && <FooterEntry label={t('CAKE to burn:')} value={raisingTokenToBurn} />} */}
          {ifo.version >= 3.2 && poolCharacteristic?.vestingInformation?.percentage && poolCharacteristic?.vestingInformation?.percentage > 0 && (
            <>
              <FooterEntry
