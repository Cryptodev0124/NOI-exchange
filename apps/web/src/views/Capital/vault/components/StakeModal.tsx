...
const StakeModal: React.FC<React.PropsWithChildren<DepositModalProps>> = ({
  vault,
  addLiquidityUrl = "",
  enablePendingTx,
  onConfirm,
  ...
  const fullBalance = useMemo(() => {
    return getFullDisplayBalance(new BigNumber(vault?.userData?.dcp.balance ?? 0), 9);
  }, [vault]);

  const needEnable = useMemo(() => {
    if (vault?.userData?.dcp.allowance) {
      const amount = getDecimalAmount(new BigNumber(val), 9);
      return amount.gt(vault?.userData?.dcp.allowance);
    }
    return false;
  }, [vault, val]);
  ...
  const willGetValueBN = lpTokensToStake.div(dcpPrice)
  ...
  const handleChange = useCallback(
    ...
    [setVal, setValUSDPrice, dcpPrice]
  );
  ...
  const handleSelectMax = useCallback(() => {
    ...
    setValUSDPrice(USDPrice);
  }, [fullBalance, setVal, setValUSDPrice, dcpPrice]);
  ...
  const handlePercentInput = useCallback(
    ...
    [fullBalanceNumber, dcpPrice]
  );

  return (
    <Modal title={t("Stake %symbol%", {symbol: "DCP"})} onDismiss={onDismiss}>
