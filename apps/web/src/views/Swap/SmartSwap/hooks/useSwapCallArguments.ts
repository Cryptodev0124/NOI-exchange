...
export function useSwapCallArguments(
  trade: TradeWithStableSwap<Currency, Currency, TradeType> | null, // trade to execute, required
...
function swapCallParameters(
  ...
  const amountIn = toHex(Trade.maximumAmountIn(trade, options.allowedSlippage))
  const amountOut = toHex(Trade.minimumAmountOut(trade, options.allowedSlippage))

  const path: `0x${string}`[] = trade.route.path.map((token, index) => {
    if (
  ...
  let args: (Hex | Hex[])[]
  let value: Hex
  const flag: Hex[] = trade.route.pairs.map((pair) => {
