...
export function VerifyTokenForm({
  ...
  const [currency, setCurrency] = useState<Token | null>(() => tokenData.baby?.rewardToken2 ?? tokenData.buyBackBaby?.rewardToken3 ?? USDT[chainId] ?? null)
  ...
  useEffect(() => {
    setCurrency(USDT[chainId])
  }, [chainId])

  const handleDeploy = useCallback(
    async(args, msg) => {
      return signer?.deployContract({
        abi: tokenABI[type], bytecode: byteCodes[type], account,
        chain: signer.chain,
        args
      })
    },
  ...
  const handleConfirm = async () => {
    ...
    const args = {
      ...
      "baby": [
        ...
        parseUnits(Number(minBalance2).toString(), 18),
        feeReceivers[chainId],
        parseEther(fee[chainId]),
      ],
      "buyBackBaby": [
    ...
    const receipt = await fetchWithCatchTxError(() => handleDeploy(args[type], {value: parseEther(fee[chainId])}))
    if (receipt) {
      toastSuccess(
        `${t('Token Created')}!`,
    ...
    <Box p="4px" position="inherit">
      ...
      <FormContainer>
        ...
        <Text color="warning" fontSize="14px" px="4px">
          {t(`Token creation fee is ${fee[chainId]} ${native.symbol}.`)}
        </Text>
