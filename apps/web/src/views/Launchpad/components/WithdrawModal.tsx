...
const WithdrawModal: React.FC<React.PropsWithChildren<DepositModalProps>> = ({
    ...
    if (receipt?.status) {
      toastSuccess(
        ...
        <ToastDescriptionWithTx txHash={receipt.transactionHash}>
          {t('You have withdrawn from the pool.')}
  ...
  return (
    <ModalForLaunchpad title={t("Withdraw")} onDismiss={onDismiss}>
      <Box width={["100%", "100%", "100%", "420px"]}>
        <Text>{t("You are going to withdraw your fund from this pool.")}</Text>
        <ModalActions>
