...
const DepositModal: React.FC<React.PropsWithChildren<DepositModalProps>> = ({
  ...
  const willGetValueBN = token ? lpTokensToStake.times(data.rate).div(10**token.decimals) : BIG_ZERO
      ...
      <ModalBody width={["100%", "100%", "100%", "420px"]}>
        ...
        {data.presaleType === "standard" && <Flex mt="24px" alignItems="center" justifyContent="space-between">
          ...
          <Flex
              alignItems="center"
            >
              <AnnualRoiDisplay>{formattedAnnualRoi} {token?.symbol ?? "Unknown"}</AnnualRoiDisplay>
            </Flex>
        </Flex>}
        <ModalActions>
