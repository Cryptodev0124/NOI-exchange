...
import { BUSD, GTOKEN, USDT} from '@pancakeswap/tokens'
import { useMemo } from 'react'
import useSWR from 'swr'
import getLpAddress from 'utils/getLpAddress'
import { multiplyPriceByAmount } from 'utils/prices'
import { isChainTestnet, publicClient } from 'utils/wagmi'
...
export const usePriceByPairs = (currencyA?: Currency, currencyB?: Currency) => {
  ...
  const pair = new Pair(
    CurrencyAmount.fromRawAmount(token0, reserve0?.toString() ?? "1"),
    CurrencyAmount.fromRawAmount(token1, reserve1?.toString() ?? "1"),
  )
