import { getFarmConfig, supportedChains } from '@pancakeswap/farms/constants'
...
import { publicClient, chains } from 'utils/wagmi'
...
>(
  ...
  async ({ pids, chainId, flag = 'pkg' }, { dispatch, getState }) => {
    ...
    if (!chain || !supportedChains.includes(chain.id)) throw new Error('chain not supported')
    try {
...
export const fetchFarmUserDataAsync = createAsyncThunk<
  ...
  { account: `0x${string}`; pids: number[]; proxyAddress?: string; chainId: number },
  {
    state: AppState
  }
>(
  ...
  async ({ account, pids, proxyAddress, chainId }, { dispatch, getState }) => {
    ...
    const farmsConfig = await getFarmConfig(chainId)
    const farmsCanFetch = farmsConfig.filter(
      (farmConfig) => pids.includes(farmConfig.pid) && poolLength[0].result > farmConfig.pid,
    )
