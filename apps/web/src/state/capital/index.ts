...
const fetchCapitalPublicData = async ({ ids, chainId }): Promise<[SerializedBond[], SerializedVault]> => {
  ...
  const marketPrice = reserves[1] / reserves[0] * 1000

  const bonds = await fetchBonds(new BigNumber(marketPrice), bondsCanFetch, chainId)

  const vault = await fetchVault(BigInt(marketPrice), chainId)
...
export const fetchCapitalUserDataAsync = createAsyncThunk<
  ...
  { account: string | null | undefined; ids: number[]; chainId: number },
  {
    state: AppState
  }
>(
