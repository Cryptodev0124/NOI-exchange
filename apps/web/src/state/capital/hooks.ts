...
export const useCapital = (): SerializedCapitalState | undefined => {
  const { chainId } = useActiveChainId()
  return useSelector(useMemo(() => capitalSelector(chainId), [chainId]))
}

export const useBondFromId = (id: number): SerializedBond | undefined => {
  const bondFromPid = useMemo(() => makeBondFromIdSelector(id), [id])
  return useSelector(bondFromPid)
}

export const useBondFromName = (name: string): SerializedBond | undefined => {
  const bondFromName = useMemo(() => bondFromNameSelector(name), [name])
  return useSelector(bondFromName)
}

export const useBondUser = (id): SerializedBondUserData | undefined => {
