...
import { GTOKEN, USDC, USDT, shimmer2Tokens } from '@pancakeswap/tokens'
import tryParseAmount from '@pancakeswap/utils/tryParseAmount'
import {IPancakePairABI} from 'config/abi/IPancakePair'
...
export function queryParametersToSwapState(
  ...
  console.log(parsedQs.inputCurrency, isAddress(parsedQs.inputCurrency), nativeSymbol)
...
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    ...
    const parsed = queryParametersToSwapState(query, undefined, GTOKEN[chainId]?.address ?? USDT[chainId]?.address)
...
export const useFetchPairPrices = ({
  ...
  const { chainId } = useActiveChainId()
  const [pairId, setPairId] = useState<`0x${string}` | undefined>()
  const [isLoading, setIsLoading] = useState(false)
  const pairData = useSelector(pairByDataIdSelector({ pairId: pairId as string, timeWindow }))
  const derivedPairData = useSelector(derivedPairByDataIdSelector({ pairId: pairId as string, timeWindow }))
  ...
  useEffect(() => {
    const fetchDerivedData = async () => {
      ...
      try {
        ...
        if (derivedData) {
          ...
          dispatch(updateDerivedPairData({ pairData: normalizedDerivedData, pairId: pairId as string, timeWindow }))
        } else {
          dispatch(updateDerivedPairData({ pairData: [], pairId: pairId as string, timeWindow }))
        }
      } catch (error) {
    ...
    const fetchAndUpdatePairPrice = async () => {
      setIsLoading(true)
      const { data } = await fetchPairPriceData({ pairId: pairId as string, timeWindow })
      if (data) {
        ...
        try {
          if (!pairId) return
          pairTokenResults = await client.multicall({
            contracts: [
              {
                abi: IPancakePairABI,
                address: pairId,
        ...
        if (newPairData.length > 0 && hasEnoughLiquidity) {
          dispatch(updatePairData({ pairData: newPairData, pairId: pairId as string, timeWindow }))
          setIsLoading(false)
        } else {
          console.info(`[Price Chart]: Liquidity too low for ${pairId}`)
          dispatch(updatePairData({ pairData: [], pairId: pairId as string, timeWindow }))
          fetchDerivedData()
        }
      } else {
        dispatch(updatePairData({ pairData: [], pairId: pairId as string, timeWindow }))
    ...
    const updatePairId = () => {
      try {
        ...
        if (pairAddress !== pairId) {
          setPairId(pairAddress as `0x${string}`)
        }
      } catch (error) {
        setPairId(undefined)
      }
  ...
  const pairPrices = hasNoDirectData && hasNoDerivedData ? [] : undefined
