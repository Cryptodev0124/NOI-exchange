...
const FixedContainer = styled.div<{ showMenu: boolean; height: number }>`
  position: fixed;
  top: ${({ showMenu, height }) => (showMenu ? 0 : `-${height + 15}px`)};
  left: 0;
  transition: top 0.2s;
        ...
        <Wrapper>
          <FixedContainer showMenu={showMenu} height={totalTopMenuHeight}>
            ...
            <StyledNav>
              ...
              <Flex>
                <div style={{ background: '#2e2e2e', borderRadius: '8px', height: '48px', marginRight: '8px' }}>
                  {leftSide}
                </div>
                <div style={{ background: '#2e2e2e', borderRadius: '8px', height: '48px', display: 'flex', alignItems: 'center' }}>
                  {!isHome && rightSide}
                </div>
                {isHome && <NextLinkFromReactRouter to="/swap">
                  <Button variant='secondary' scale="md" width="150px" height="40px" mr="8px">{'Launch App'}</Button>
                </NextLinkFromReactRouter>}
              </Flex>
              {/* </Flex> */}
            </StyledNav>
          </FixedContainer>
          <BodyWrapper mt={!isHome ? `${totalTopMenuHeight + 50}px` : `0px`}>
            {!isHome && <Panel
              isPushed={isPushed}
              isMobile={isMobile}
              showMenu={showMenu}
